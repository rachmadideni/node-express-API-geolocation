...
exports.load = (req, res) => {
  ...
  const exec = async (_) => {
    const unik = await extract_fid_unik();
    const koord_master = [];

    for (let i = 0; i < unik.length; i++) {
    	const featureId = unik[i];
      const koord_arr = [];
    	const koord = await extract_kordinat(unik[i]);
  ...
  exec().then((featureUnik) => {
    const result = geojson.parse(featureUnik, { LineString: 'line' });
...
exports.loadAttributes = (req, res) => {
  ...
  db.query(q, { replacements: { idsung }, type: db.QueryTypes.SELECT })
    .then((result) => {
      if (result) {
        response.ok(result, res);
      } else {
...
exports.loadAttributesById = (req, res) => {
  ...
  const findAttribute = async (featureId) => {
    const q = `SELECT 
						...
						WHERE a.featureId=:featureId
						LIMIT 1`;
    const result = db.query(q, { replacements: { featureId }, type: db.QueryTypes.SELECT });
...
exports.addRiver = (req, res) => {
  ...
  const { features } = req.body; // features dari komponen draw
...
exports.deleteRiver = (req, res) => {
  ...
  const isFeatureIdExist = async (featureId) => {
    const data = await sungai_geom.findOne({ where: { featureId } });
    const { idsung } = data.dataValues;
  ...
  const doDeleteAction = async (featureId) => {
    ...
    const { idsung } = objekHapus;
    // console.log('objekHapus:',objekHapus);
    const deletedRiver = await sungai_geom.destroy({ where: { featureId } }); // return berapa row yg dihapus
...
exports.replaceMapRiver = (req, res) => {
  ...
  const { jenis_sungai } = properties[0];
