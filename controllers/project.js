...
exports.replaceCoordinat = (req, res) => {
  ...
  const hapusData = async (featureId) => await clearProjectByFeatureId(featureId);
...
exports.addNewProject = (req, res) => {
  ...
  const addProject = async ({ featureId, geometry }) => {
    const isProjectInserted = await test_project.create({ featureId, geometry });
...
exports.getProjectAttributes = (req, res) => {
  ...
  const doGet = async (featureId) => {
    ...
    return await db.query(q, { replacements: { featureId }, type: db.QueryTypes.SELECT });
  };
...
exports.getAllProjectAttributes = (req, res) => {
  ...
  test_project.findAll({
    ...
    geojson.parse(output_array, { GeoJSON: 'geometry' }, (result) => {
