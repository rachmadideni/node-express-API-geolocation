...
const generateInsertValues = async (features, properties) => {
  ...
  const { jenis_sungai } = properties;
  const { keterangan } = properties;
...
const isFeatureIdExists = async (features) => {
  ...
  try {
    const { featureId } = features[0].properties;
    if (featureId) {
...
const updateRiverProperty = async (featureId, property) => {
  ...
  const getFieldsToUpdate = (fields) => {
    ...
    const {
      kecamatan, idsung, jenis_sungai, keterangan,
    } = fields;
    const obj = {
      idkecm: kecamatan,
      idsung,
  ...
  return await sungai_geom.update(fieldsToUpdate, { where: { featureId } });
